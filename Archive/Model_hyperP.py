...
epoch = [10]#0, 200, 400, 800]#200
batch_size = 2
learn_rate = [0.001]#, 0.0006, 0.0001, 0.00006, 0.00001, 0.000006, 0.000001, 0.0000006, 0.0000001]
neurons = [0]#, 128, 256, 512, 768, 1024, 2046]
l2reg = [0.000001]#, 0.0006, 0.0001, 0.00006, 0.00001, 0.000006, 0.000001, 0.0000006, 0.0000001]
conv_layer = [4]
Drop = [0]
count = 0
Results=list()
            ...
            for ep in epoch:             
                ...
                def CNN():
                    ...
                    for i in range(2, 6, 1):
                        model.add(Conv2D(8*2**i, kernel_size = (3,3), strides=1, padding='valid'))
                        model.add(MaxPooling2D((2,2)))
                    #model.add(Conv2D(2*8*2**i, kernel_size = (3,3), strides=1, padding='valid'))
                ...
                plt.legend()
                plt.show()
                tmp_row = model.evaluate(X_test, y_test, batch_size=batch_size, verbose=0) # evaluate the model to get the loss and accuracy
                tmp_row1 = model.evaluate(X_train, y_train, batch_size=batch_size, verbose=0)
                tmp.append((tmp_row,tmp_row1))
...
#Adding features
test2 = dx_dy(test1, ksize=5)
#Calculate missing Incident Angles
test3 = predict_IAngle(test2)
#Apply scaler based on Incident Angle of aquisition.
test4 = Incident_Angle_scaling(test3, scaler=0.015)
#Data wrangling
X_test, X_test_extreme, X_Angle_test = data_wrangling(test4, y=False)
#scaling
X_test5 = Scaling(X_test)
X_test5=X_test5[:,:,:,0:2]
##Image augmentation
#gen_flow = Image_Augmentation(X_Train, X_Angle_Train, y_Train)
y_pred = model.predict(X_test5, batch_size=32, verbose=1)
